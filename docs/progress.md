...
- UX polish (post-v2.1, not a plan phase): replaced the free-text origin/destination inputs (Trip advisor) and the leg/transfer/final inputs (Connection mode) with searchable `st.selectbox` pickers populated from the loaded dataset — the user types to filter and chooses valid stations, no exact spelling. Surfaced a real bug this fixes: the old default "Berlin Hbf" is absent from the real dataset (which uses "Berlin Hauptbahnhof"), so it silently returned no-data. New pure helpers `station_names` and `default_option_index` in `data_loader` (6 tests). Risk engine and StationStats model unchanged; no-data/fallback path retained. 157/157 passing.

- UX/trust polish (post-v2.1, not a plan phase): (1) added a visible "Travel date" date picker to the Trip advisor form, surfaced in the result and captioned as trip context (weather/live checks reflect current conditions; backend calcs still use time only — no engine change). (2) Added a "What this uses — data sources & limitations" info box near the top listing historical reliability (piebro/deutsche-bahn-data, CC BY 4.0; reflects real vs sample), weather (Open-Meteo + fallback), live delay (db.transport.rest, fail-closed), and construction (manual prototype flag only). New pure helper `data_source_notes(using_real_data)` in `ui_helpers` (4 tests). StationStats model, risk thresholds, and risk engine unchanged. 161/161 passing.
